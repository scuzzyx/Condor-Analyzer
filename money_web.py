...
st.markdown("<h2 style='font-size: 2.2rem; margin-bottom: 0rem;'>🛡️ Aegis Option Scanner | Delta & Volatility Edge</h2>", unsafe_allow_html=True)
...
def fetch_macro_data():
    ...
    try:
        req = urllib.request.Request("https://production.dataviz.cnn.io/index/fearandgreed/graphdata", headers={'User-Agent': 'Mozilla/5.0'})
...
with tab_scanner:
    ...
    for idx, symbol in enumerate(selected_tickers):
        try:
            # 1. PULL FROM LOCAL CACHE (NO BLOCKS)
